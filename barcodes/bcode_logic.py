...
def update_ean(context, node, newtext):
    if node is None:
        return
    if not hasattr(node, "mktext") or not hasattr(node, "mkbarcode") or getattr(node, "mkbarcode") != "ean":
        return

def update_qr(context, node, newtext):
    if node is None:
        return
    if not hasattr(node, "mktext") or not hasattr(node, "mkbarcode") or getattr(node, "mkbarcode") != "qr":
        return